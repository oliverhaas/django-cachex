...
def _stub_methods() -> set[str]:
    ...
    # Pull `def name(` from inside the RustValkeyDriver class block. The block
    # starts at `class RustValkeyDriver:` and ends at the next top-level def.
    start = text.index("class RustValkeyDriver:")
...
def test_stub_covers_runtime_surface():
    runtime = {m for m in dir(RustValkeyDriver) if not m.startswith("_")}
