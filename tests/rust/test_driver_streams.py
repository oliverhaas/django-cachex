def test_xadd_xlen(driver):
    id1 = driver.xadd("s", "*", [("field", b"v1")])
    id2 = driver.xadd("s", "*", [("field", b"v2")])
    assert driver.xlen("s") == 2
    assert id1 < id2


def test_xrange(driver):
    driver.xadd("s", "*", [("field", b"v1")])
    driver.xadd("s", "*", [("field", b"v2")])
    entries = driver.xrange("s", "-", "+")
    assert len(entries) == 2
    # XRANGE returns each entry as [id_bytes, [field_bytes, value_bytes, ...]].
    assert entries[0][1] == [b"field", b"v1"]
    assert entries[1][1] == [b"field", b"v2"]


def test_xrange_with_count(driver):
    for i in range(5):
        driver.xadd("s", "*", [("i", str(i).encode())])
    entries = driver.xrange("s", "-", "+", count=2)
    assert len(entries) == 2


def test_xread(driver):
    driver.xadd("s", "*", [("field", b"v1")])
    result = driver.xread(["s"], ["0"])
    assert result is not None
    assert len(result) == 1


def test_xtrim(driver):
    for i in range(10):
        driver.xadd("s", "*", [("i", str(i).encode())])
    driver.xtrim("s", max_len=3)
    assert driver.xlen("s") == 3


def test_xgroup_create_and_xreadgroup(driver):
    driver.xadd("s", "*", [("field", b"v1")])
    driver.xgroup_create("s", "g1", "0", mkstream=False)
    result = driver.xreadgroup("g1", "consumer1", ["s"], [">"])
    assert result is not None


def test_xack(driver):
    driver.xadd("s", "*", [("field", b"v1")])
    driver.xgroup_create("s", "g1", "0", mkstream=False)
    result = driver.xreadgroup("g1", "consumer1", ["s"], [">"])
    # RESP3 returns XREADGROUP as a map: {stream: [[id, [field, value]]]}
    entries = result[b"s"]
    msg_id = entries[0][0].decode()
    assert driver.xack("s", "g1", [msg_id]) == 1


def test_xpending(driver):
    driver.xadd("s", "*", [("field", b"v1")])
    driver.xgroup_create("s", "g1", "0", mkstream=False)
    driver.xreadgroup("g1", "consumer1", ["s"], [">"])
    result = driver.xpending("s", "g1")
    assert result[0] == 1  # 1 pending message


def test_xgroup_create_mkstream(driver):
    # Stream doesn't exist yet — mkstream=True creates it.
    driver.xgroup_create("new_stream", "g", "0", mkstream=True)
    assert driver.xlen("new_stream") == 0
