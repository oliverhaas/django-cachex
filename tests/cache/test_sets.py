...
class TestSetOperations:
    ...
    def test_sdiffstore_with_keys_version(self, cache: RespCache):
        # Use hash tags {foo} to ensure keys are on same cluster slot
    ...
    def test_interstore(self, cache: RespCache):
        # Use hash tags {foo} to ensure keys are on same cluster slot
        cache.sadd("{foo}1", "bar1", "bar2")
        cache.sadd("{foo}2", "bar2", "bar3")
    ...
    def test_spop(self, cache: RespCache):
        cache.sadd("foo", "bar1", "bar2")
    ...
    def test_srandmember(self, cache: RespCache):
        cache.sadd("foo", "bar1", "bar2")
    ...
    def test_sscan_with_match(self, cache: RespCache):
    ...
    def test_sscan_iter_with_match(self, cache: RespCache):
        # SSCAN match operates on raw bytes, not deserialized values.
        # Since values are serialized, match patterns won't work for strings.
        pytest.skip("SSCAN match doesn't work with serialized values")
...
class TestAsyncSetStoreOperations:
    ...
    @pytest.mark.asyncio
    async def test_asdiffstore_with_keys_version(self, cache: RespCache):
...
class TestAsyncRandomMembers:
    ...
    @pytest.mark.asyncio
    async def test_aspop(self, cache: RespCache):
    ...
    @pytest.mark.asyncio
    async def test_asrandmember(self, cache: RespCache):
