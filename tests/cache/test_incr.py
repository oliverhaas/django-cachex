...
class TestIncrementOperations:
    ...
    def test_increment_by_one(self, cache: RespCache):
        cache.set("counter", 5)
        cache.incr("counter")
        assert cache.get("counter") == 6

    def test_increment_by_custom_amount(self, cache: RespCache):
        cache.set("counter", 10)
        cache.incr("counter", 7)
        assert cache.get("counter") == 17

    def test_increment_chain(self, cache: RespCache):
        cache.set("chain", 0)
        cache.incr("chain")
        cache.incr("chain", 4)
        cache.incr("chain", 5)
...
class TestIncrementWithoutTimeout:
    ...
    def test_increment_persistent_key(self, cache: RespCache):
        cache.set("persistent", 100, timeout=None)
        cache.incr("persistent")
        assert cache.get("persistent") == 101

    def test_increment_persistent_by_amount(self, cache: RespCache):
