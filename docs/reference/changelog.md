...
- **Async ext methods on LocMem and Database.** The full async data-structure surface (`alpush`, `ahset`, `azadd`, `attl`, `aexpire`, …) is now available on `LocMemCache` (direct sync calls — in-memory, no I/O to offload) and `DatabaseCache` (via ``sync_to_async``, the same path Django uses for ``BaseCache.aget``). They no longer raise `NotSupportedError` from async views.
