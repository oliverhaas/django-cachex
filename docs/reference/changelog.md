...
- `StreamCache.set_many()` publishes one `set_many` stream entry carrying every key instead of one `set` entry per key, so a large `set_many()` no longer exhausts the publish budget and drops writes. Pods on an older release ignore the new entry; upgrade every pod sharing a `stream_key` together.
- `add()` and `set()` with `nx`, `xx` or `get` and `timeout=0` send a single `SET ... PXAT 1` instead of `SET` followed by `UNLINK`, which left a window in which the key existed without a TTL (forever, if the process died in between). `PXAT` needs Redis 6.2+, the same floor `SET ... GET` already had; the documented minimum Redis version is now 6.2.
...
- `memory_usage(key, *, samples=None)` and `amemory_usage()` return the bytes a key and its value take on the server (`MEMORY USAGE`), `None` for a missing key, on the redis-py, valkey-py and valkey-glide backends including cluster and Sentinel. Also available on pipelines. `TrackingCache` delegates it to its transport; `LocMemCache`, `DatabaseCache` and `StreamCache` raise `NotSupportedError`, as do the new `amemory_usage()` and `alargest_keys()` defaults on `BaseCachex`.
- `largest_keys(pattern="*", count=10, *, samples=None, itersize=None)` and `alargest_keys()` return the `count` largest keys matching `pattern` as `(key, bytes)` pairs, largest first, scanning with `iter_keys()` and pipelining `MEMORY USAGE` in batches. `count=0` returns `[]` without scanning and a negative count raises `ValueError`.
...
- `StreamCache` gained the async twins `attl()`, `apttl()`, `apersist()`, `aexpire()`, `akeys()`, `ascan()`, `aiter_keys()`, `adelete_pattern()` and `aget_or_set()`; they raised `NotSupportedError` while the sync methods worked.
...
- `TrackingCache` and `StreamCache` log the traceback once per outage of their listener, consumer or publish path and a one-line warning per later attempt, instead of a full traceback every second. A `StreamCache` whose `transport` alias is misconfigured is retried with the consumer's backoff rather than restarted, replay included, by every cache operation.
...
- `StreamCache.set()` no longer raises `ValueError("Semaphore released too many times")` when a publish races a consumer restart: the publish path released whichever budget semaphore was current instead of the one it acquired, and left a stale pending mark behind.
...
- valkey-glide: a `LOCATION` the driver cannot dial (a unix-socket URL, a schemeless `host:port`, a bare hostname or a non-numeric port) raises `ImproperlyConfigured` instead of silently connecting to `localhost:6379`. `xtrim()` without `maxlen` or `minid` and a pipelined `hmget()` without fields raise `ValueError` before anything reaches the wire; the pipelined `xread()` returns `None` for an empty read like the direct call; `aget_many()` raises `CachexError` when the server discards its TTL batch, as `get_many()` does, instead of serving every key as fresh; and `aclose()` drops the per-loop registry bookkeeping it used to leave behind.
...
- Sentinel `aclose()` closes the discovery clients through the pool's own Sentinel manager, so they no longer leak when a different adapter instance performs the close (asgiref hands each task a fresh instance).
