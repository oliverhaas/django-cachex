...
class KeyValueCache(BaseCache):
    ...
    def _serializers(self):
        """Delegate to CacheClient's serializers (for backwards compatibility)."""
        return self._cache._serializers

    @property
    def _compressors(self):
        """Delegate to CacheClient's compressors (for backwards compatibility)."""
        return self._cache._compressors

    @property
    def _compressor(self):
        """Primary compressor (for backwards compatibility)."""
        return self._cache._compressors[0] if self._cache._compressors else None
    ...
    def set_many(self, data, timeout=DEFAULT_TIMEOUT, version=None):
        ...
        if not data:
            return []
        safe_data = {
            self.make_and_validate_key(key, version=version): value
