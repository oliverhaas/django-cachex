...
class _RespPipelineCommandsProtocol(Protocol):
    ...
    def memory_usage(self, key: str, *, samples: int | None = None) -> Any: ...
...
class RespAdapterProtocol(Protocol):
    ...
    # ``async def`` even where the body is sync: glide's async client is built asynchronously.
    ...
    # MEMORY USAGE: bytes for the key and its value, None for a missing key.
