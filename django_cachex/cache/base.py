...
def _install_async_delegates(cls: type) -> None:  # noqa: C901
    """For each ext-method ``a<name>`` declared on :class:`BaseCachex`, install an
    ``async def`` wrapper on ``cls`` that delegates to the sync ``<name>``.

    Use this in non-RESP backends whose async story is "blocking sync call,
    no I/O concurrency benefit" — in-memory backends (LocMem, Stream) and
    thin layers (Compat, Tiered). RespCache has its own real async impls
    on every method and doesn't need this.

    A wrapper is installed only when:

    - ``cls`` (or a non-:class:`BaseCachex` ancestor) overrides the sync
      method — so unsupported sync methods stay unsupported async too,
    - ``cls`` doesn't already define the async method itself — explicit
      overrides win.
    """
    for async_name, base_async in vars(BaseCachex).items():
        if not async_name.startswith("a") or async_name.startswith("__"):
            continue
        if not callable(base_async):
            continue
        sync_name = async_name[1:]
        if not hasattr(BaseCachex, sync_name):
            continue
        if async_name in cls.__dict__:
            continue
        sync_method = None
        for ancestor in cls.__mro__:
            if ancestor is BaseCachex or ancestor is BaseCache or ancestor is object:
                break
            if sync_name in ancestor.__dict__:
                sync_method = ancestor.__dict__[sync_name]
                break
        if sync_method is None or not callable(sync_method):
            continue

        def _make_wrapper(_sync: Any, _name: str) -> Any:
            async def wrapper(self: Any, *args: Any, **kwargs: Any) -> Any:
                return _sync(self, *args, **kwargs)

            wrapper.__name__ = _name
            wrapper.__qualname__ = f"{cls.__name__}.{_name}"
            wrapper.__doc__ = f"Async fallback delegating to :meth:`{cls.__name__}.{_name[1:]}`."
            return wrapper

        setattr(cls, async_name, _make_wrapper(sync_method, async_name))
